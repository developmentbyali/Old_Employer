...
export default function EmployerSelect({ value = '', onChange }) {
  ...
  return (
    <div ref={containerRef} style={{position:'relative',width:220}}>
      ...
      >
        <div style={{flex:1,color:'#9B9B9B',fontWeight:500,fontSize:16,textAlign:'left',display:'flex',alignItems:'center',justifyContent:'flex-start'}}>{EMPLOYERS.find(c=>c.id===value)?.name || 'Employer'}</div>
